...
TEST_CASE("Vectorize smul", "[Vectorize]") {
    ...
    Dynamo::Vectorize::smul(a, b, c, 10);
...
TEST_CASE("Vectorize vadd", "[Vectorize]") {
    ...
    Dynamo::Vectorize::vadd(a, b, c, 10);
...
TEST_CASE("Vectorize vsma", "[Vectorize]") {
    ...
    Dynamo::Vectorize::vsma(a, b, c, 10);
...
TEST_CASE("Vectorize vclamp", "[Vectorize]") {
    float a[] = {-2, 0, 1, 4, 0.5, 1, 5, -0.5, -3, -0.25};
    float lo = -1;
    float hi = 1;
    float c[] = {1, 4, 7, 9, 0, 3, 2, 5, 2, 4};

    Dynamo::Vectorize::vclamp(a, lo, hi, c, 10);
