...
namespace Dynamo {
     ...
     *
     */
    constexpr std::array<unsigned, 32> DE_BRUJIN_TABLE = {
    ...
    constexpr unsigned reverse_bits(unsigned x) {
        return (BIT_REVERSAL_TABLE[x & 0xFF] << 24) | (BIT_REVERSAL_TABLE[(x >> 8) & 0xFF] << 16) |
               (BIT_REVERSAL_TABLE[(x >> 16) & 0xFF] << 8) | (BIT_REVERSAL_TABLE[(x >> 24) & 0xFF]);
    }
