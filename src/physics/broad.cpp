...
namespace Dynamo::Physics {
	bool Projection::is_overlapping(const Projection &b) {
		return (min <= b.max && b.min <= max);
	}

	float Projection::get_range() {
		return abs(max - min);
	}
	...
	float AABB::get_volume() {
		return get_width() * get_height();
	}

	void AABB::add_volume(const AABB &b) {
	...
	bool AABB::is_colliding(const AABB &b) {
